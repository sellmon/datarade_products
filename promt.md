...
Задача: заполнить карточку дата-сета, который могут купить клиенты для тренировки своей модели. Дата-сет должен базироваться на фото, которые можно сделать через камеру мобильного телефона.

Вот тема дата-сета: Facial Expression Datasets: Collecting selfies showing different emotions like happiness, sadness, anger, surprise for emotion detection models.
